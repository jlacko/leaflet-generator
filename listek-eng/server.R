    ...
    vystup <- leaflet(data = dataset) %>% 
      addProviderTiles('CartoDB.Positron',
                       options = leafletOptions(opacity = .75,
                                                attribution = '© <a href="https://www.openstreetmap.org/copyright", target="_blank">OpenStreetMap</a> contributors © <a href="https://carto.com/attribution/", target="_blank">CARTO</a> — created with <a href="https://www.jla-data.net/eng/jla-leaflet-generator/", target="_blank">JLA Leaflet Generator</a>')) %>% 
